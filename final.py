...
class HealthInsuranceAnalysisApp:
    def __init__(self):
        ...
        self.pca_fa_button = tk.Button(self.root, text="PCA and Factor Analysis", command=self.perform_pca_fa)
        self.pca_fa_button.place(relx=0.5, rely=0.6, anchor=tk.CENTER)

        self.cluster_button = tk.Button(self.root, text="Hierarchical and Non-hierarchical Clustering", command=self.perform_clustering)
    ...
    def open_regression_options(self):
        ...
        regression_image_label.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
    ...
    def perform_pca_fa(self):
        subprocess.Popen(["python3", "pca analysis.py"])

    def perform_clustering(self):
        try:
            hierarchial_process = subprocess.Popen(["python3", "Swathi.py"])
            hierarchial_process.wait()
            non_hierarchical_process = subprocess.Popen(["python3", "non-hierarchial.py"])
            non_hierarchical_process.wait()
        except Exception as e:
            messagebox.showerror("Error", f"An error occurred while performing clustering: {e}")
